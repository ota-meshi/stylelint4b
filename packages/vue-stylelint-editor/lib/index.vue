<template>
  ...
  >
    <transition name="vue-stylelint-editor-fade" @before-enter="fadeIn">
      ...
      <div v-else key="placeholder" class="vue-stylelint-editor-swap-container">
        <code class="vue-stylelint-editor-placeholder-code">{{ code }}</code>
        <transition name="vue-stylelint-editor-fade">
          ...
          >
            <div class="vue-stylelint-editor-placeholder-loading-icon">
              <div />
              <div />
              <div />
            </div>
            <div class="vue-stylelint-editor-placeholder-loading-message">
              Now loading...
            </div>
          </div>
        </transition>
  ...
  methods: {
    ...
    async lint() {
      ...
      try {
        ...
        this.fixedCode =
          ret.output && typeof ret.output === "string" && ret.output !== code
            ? ret.output
            : code;
  ...
  if (result.needlessDisables && result.needlessDisables[0]) {
    warnings = [
      ...
      ...result.needlessDisables[0].ranges.map((range) => ({
        ...
        text: `unused rule: ${range.unusedRule}, start line: ${range.start}${
